...
class Day05Test extends FunSuite {
  ...
  test("test jumps") {
    ...
    val state1 = Day05.MemoryState(memory.toArray, input = Some(3), pointer = 0)
    Day05.evaluateProgram(state1)
    state1.output should be (Some(999))

    val state2 = Day05.MemoryState(memory.toArray, input = Some(8), pointer = 0)
    Day05.evaluateProgram(state2)
    state2.output should be (Some(1000))

    val state3 = Day05.MemoryState(memory.toArray, input = Some(10), pointer = 0)
